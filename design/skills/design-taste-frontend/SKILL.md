...
description: Anti-slop frontend skill for new landing pages, portfolios, and greenfield UIs. The agent reads the brief, infers the right design direction, and ships interfaces that do not look templated. Real design systems when applicable, strict pre-flight check. To upgrade an existing UI codebase, use redesign-existing-projects.
---
