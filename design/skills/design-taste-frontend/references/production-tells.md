# Production-Test Tells: Full Catalog

These patterns came out of real LLM-generated landing-page tests. They are the signatures the model defaults to when it tries to "look designed." Treat them as hard bans unless the brief explicitly calls for one. SKILL.md Section 7.C carries the one-line summary; this file has the examples and the narrow exceptions.
...
* **NO `Scroll · 001 Capabilities`-style scroll cues.** Section-number prefixes on scroll cues are doubly wrong; scroll cues themselves are banned (see Locale, time, scroll cues below).
...
* **ZERO decorative colored status dots by default.** A colored dot before "ONE Q4 SLOT OPEN", before every nav link, list row, badge, or task row is a Tell. Acceptable only when the dot conveys actual semantic state (a live indicator on actual server status, a live availability flag) and is limited to one per page section.

## Em-dashes & typography flourishes
* **NO em-dash (`—`) as a design element OR anywhere else.** See SKILL.md Section 7.D for the complete, non-negotiable ban. The em-dash character is forbidden in headlines, eyebrows, pills, body copy, quotes, attribution, captions, button text, and alt text. Use the regular hyphen (`-`).
...
* **NO `border-t` + `border-b` on every row of a long list / spec table.** Pick one (bottom-border between rows OR top-border above the group) and use it sparsely. A 10-row spec table with hairlines under each row is the laziest layout - see SKILL.md Section 4.8 for alternative UI components.
