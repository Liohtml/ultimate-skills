...
This is a vocabulary, not a library. Know these pattern names to communicate about them, design with them in mind, and reach for them when the design read calls for them. Every pattern is still gated by the dials (SKILL.md Section 1), the motion rules (Section 5) and the AI-tell bans (Section 7). Implementation code for the scroll patterns lives in [motion-skeletons.md](motion-skeletons.md).
