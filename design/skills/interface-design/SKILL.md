...
description: Craft-first design for product UI - dashboards, admin panels, SaaS apps, internal tools, settings pages, data tables and other interactive product screens. Intent brief (who, what verb, what feel), product-domain exploration (domain, color world, signature, rejected defaults), visual-hierarchy rules with computed type scales, density in px, surface/elevation and token architecture, "use what exists" (native, headless primitives, design-system components), states and polish, plus strict design-review and deslop passes and an optional saved .interface-design/system.md. Use when designing, building, reviewing or refining product UI. Not for landing pages, marketing sites or portfolios - use design-taste-frontend (greenfield) or redesign-existing-projects (existing sites). For code-level UI lint use web-interface-review; for animation use motion-craft; for WCAG audits use accessibility-audit.
---

# Interface Design

> Adapted from [Dammyjay93/interface-design](https://github.com/Dammyjay93/interface-design) (MIT, (c) 2026 Damola Akinleye) at commit `2f9be32`. Changes: description rewritten for this marketplace; the two slash commands (`design-review`, `design-deslop`) moved into `references/` as review modes; system template and examples moved into `references/`; image-pass and motion notes point to sibling skills; `agents/openai.yaml` dropped.
...
**Not for:** Landing pages, marketing sites, campaigns, brand-only work. Use `design-taste-frontend` (new sites) or `redesign-existing-projects` (existing sites) for those.
...
Motion should be felt, not watched. Fast, purposeful, and *never* in the way. For exact curves, recipes and a strict motion review, use `motion-craft`.
...
- **Design review** — strict, multi-pass craft + hierarchy review of a build with severity scoring and an explicit approval bar. Read [references/design-review.md](references/design-review.md) and follow it when the user asks for a design review, critique, or approval of product UI.
- **Deslop** — fast, diff-scoped, behavior-preserving pass that strips the tells that make UI look AI-generated. Read [references/design-deslop.md](references/design-deslop.md) when the user asks to clean up, de-slop, or polish a branch's UI.

If a user asks for design status, audit, or pattern extraction in natural language: read `.interface-design/system.md` and summarize it, check UI files against it for drift, or scan for repeated spacing/radius/color/component values and propose a system.md (template: [references/system-template.md](references/system-template.md); worked examples: [precision](references/examples/system-precision.md), [warmth](references/examples/system-warmth.md)) — perform the equivalent inline.
