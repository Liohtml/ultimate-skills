#!/usr/bin/env python3
"""CLI for image generation or editing with OpenAI GPT Image models.

Part of the ultimate-skills `image-generation` skill.

Originally from openai/skills (skills/.system/imagegen/scripts/image_gen.py,
commit 49f948f), Copyright OpenAI, licensed under the Apache License 2.0
(see ../LICENSE.txt).

Modified by ultimate-skills (2026-09):
  * no longer a Codex "explicit-only fallback"; this is the primary OpenAI path
  * default model gpt-image-2, overridable via --model or IMAGEGEN_OPENAI_MODEL
  * model-aware size validation (GPT Image 2 / 2.5 flexible WIDTHxHEIGHT sizes)
  * quality values xhigh/max accepted for gpt-image-2.5-* models
  * transparent background rejected early for gpt-image-2 (unsupported there)
  * input_fidelity dropped for gpt-image-2 (the API requires it to be omitted)
  * SDK auto-retries disabled for single calls so paid requests are not
    silently repeated; batch mode retries only rate-limit/timeouts
  * default output directory output/imagegen/ kept; dependency hint made
    package-manager neutral

Requires: `pip install openai` (Pillow optional, for --downscale-max-dim).
Environment: OPENAI_API_KEY (required for live calls, not for --dry-run).
"""
...
DEFAULT_MODEL = os.environ.get("IMAGEGEN_OPENAI_MODEL", "gpt-image-2")
...
EXTENDED_QUALITIES = ALLOWED_QUALITIES | {"xhigh", "max"}
# GPT Image 2 / 2.5 size limits (OpenAI docs, checked 2026-09): edges are
# positive multiples of 16, max edge 3840, long:short <= 3:1, total pixels
# between 655,360 and 8,294,400. Above 2560x1440 is experimental.
MIN_PIXELS = 655_360
...
def _dependency_hint(package: str, *, upgrade: bool = False) -> str:
    ...
    return (
        f"Install it into the active Python environment, e.g. `pip install {flag}{package}` "
        f"or `uv pip install {flag}{package}`."
    )
...
def _is_gpt_image_2_base(model: str) -> bool:
    """gpt-image-2 itself or a dated snapshot (gpt-image-2-YYYY-MM-DD), not 2.5."""
    return model == "gpt-image-2" or bool(re.fullmatch(r"gpt-image-2-\d{4}-\d{2}-\d{2}", model))


def _validate_size(size: str, model: str = DEFAULT_MODEL) -> None:
    if size in LEGACY_SIZES:
        return
    if not _is_gpt_image_2_family(model):
        _die(
            "size must be one of 1024x1024, 1536x1024, 1024x1536, or auto for "
            f"{model}. GPT Image 2 / 2.5 models also accept custom WIDTHxHEIGHT sizes."
        )
    m = re.fullmatch(r"(\d+)x(\d+)", size)
    if not m:
        _die("size must be auto or WIDTHxHEIGHT, e.g. 1536x864.")
    w, h = int(m.group(1)), int(m.group(2))
    if w % 16 or h % 16:
        _die("both edges must be multiples of 16 for GPT Image 2 / 2.5.")
    if max(w, h) > MAX_EDGE:
        _die(f"maximum edge is {MAX_EDGE}px.")
    if max(w, h) > 3 * min(w, h):
        _die("aspect ratio must be at most 3:1.")
    if not (MIN_PIXELS <= w * h <= MAX_PIXELS):
        _die(f"total pixels must be between {MIN_PIXELS:,} and {MAX_PIXELS:,}.")
    if w * h > 2560 * 1440:
        _warn("sizes above 2560x1440 are experimental and cost more.")


def _validate_quality(quality: str, model: str = DEFAULT_MODEL) -> None:
    allowed = EXTENDED_QUALITIES if model.startswith("gpt-image-2.5") else ALLOWED_QUALITIES
    if quality not in allowed:
        _die("quality must be one of " + ", ".join(sorted(allowed)) + f" for {model}.")
...
def _validate_model(model: str) -> None:
    if not model.startswith(GPT_IMAGE_MODEL_PREFIX):
        _die(
            "model must be a GPT Image model (for example gpt-image-2, gpt-image-2.5-flare, "
            "gpt-image-2.5-sunburst, or gpt-image-1.5)."
        )


def _validate_transparency(
    background: Optional[str], output_format: str, model: str = DEFAULT_MODEL
) -> None:
    if background == "transparent" and output_format not in {"png", "webp"}:
        _die("transparent background requires output-format png or webp.")
    if background == "transparent" and _is_gpt_image_2_base(model):
        _die(
            "gpt-image-2 does not support background=transparent. Use --model gpt-image-1.5 "
            "(or a gpt-image-2.5-* model) for native transparent cutouts."
        )
...
def _create_client():
    ...
    # max_retries=0: never silently repeat a potentially billable request.
...
def _create_async_client():
    ...
    # Batch retries are handled explicitly (rate limits / timeouts only).
    return AsyncOpenAI(max_retries=0, timeout=300)
...
async def _run_generate_batch(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        for i, job in enumerate(jobs, start=1):
            ...
            _validate_transparency(
                job_payload.get("background"), effective_output_format, str(job_payload.get("model"))
            )
    ...
    async def run_job(i: int, job: Dict[str, Any]) -> Tuple[int, Optional[str]]:
        ...
        _validate_transparency(
            payload.get("background"), effective_output_format, str(payload.get("model"))
        )
...
def _generate(args: argparse.Namespace) -> None:
    ...
    _validate_transparency(args.background, output_format, args.model)
...
def _edit(args: argparse.Namespace) -> None:
    ...
    if "input_fidelity" in payload and _is_gpt_image_2_base(args.model):
        _warn("gpt-image-2 requires input_fidelity to be omitted; dropping --input-fidelity.")
        payload.pop("input_fidelity")
    output_paths = _build_output_paths(args.out, output_format, args.n, args.out_dir)
    downscaled = None
    if args.downscale_max_dim is not None:
        downscaled = [str(_derive_downscale_path(p, args.downscale_suffix)) for p in output_paths]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate or edit images with OpenAI GPT Image models (image-generation skill)"
    )
