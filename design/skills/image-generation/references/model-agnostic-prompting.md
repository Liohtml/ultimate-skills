# Model-agnostic prompting (incl. open models: FLUX, SDXL/SD3, LoRAs)

> From [replicate/skills](https://github.com/replicate/skills) `skills/prompt-images/SKILL.md` at commit `2f36e41` (Apache-2.0, Copyright 2025 Replicate, Inc.). **Modified by ultimate-skills:** frontmatter removed, the Replicate-API model-selection section and links to the sibling `find-models` / `compare-models` skills generalized, title changed. Licensed under the Apache License 2.0 (see `../LICENSE.txt`).
...
## Choose a model from current information, not from memory

This file describes general prompting techniques. The image model landscape changes weekly. Don't assume specific models exist or are still state-of-the-art based on names you've seen before. Check the provider's current model list (see `providers.md`) and read the model's input schema or docs before running anything - supported parameters (negative prompt, guidance scale, aspect ratios, reference images) differ per model.
...
9. **Wrong model for the task**: Not every model is good at every task. Some excel at text rendering but struggle with object removal. Some are great at style transfer but poor at background editing. If a model struggles with a specific edit type, try a different one (with the user's OK, since it is another paid call) rather than fighting the prompt. See `providers.md` for routing.
