# Prompt craft for text-heavy, structured and photoreal images

> Adapted from [wuyoscar/GPT-Image2-Skill](https://github.com/wuyoscar/GPT-Image2-Skill) `skills/gpt-image/references/craft.md` at commit `05cb113` (MIT, (c) 2026 Wuyoscar). **Changes:** removed the gallery-routing section, the gallery-attribution section and all `No. N` gallery cross-references (the third-party gallery prompts are not bundled); renumbered. The principles were distilled for GPT Image 2 but transfer to Nano Banana Pro and other modern models.

Use this as a prompt-design checklist when a request is typography-, layout- or data-heavy (posters, infographics, UI mockups, research figures, multi-panel boards) or needs convincing photorealism.
...
The strongest prompts allocate space before describing surface detail.

Useful first clauses:
- `Landscape 16:9 academic concept figure…`
- `Design a 3:4 vertical poster…`
- `Create a square 3×3 grid…`
- `A 6-panel film storyboard laid out as a 3×2 grid…`
- `Create one tall manga chapter proof sheet containing 19 numbered miniature pages…`
...
Do not omit this. JSON-like structured prompts work well for premium product and food rendering. This pattern works when the output has many interacting systems: environment, subject, materials, lighting, particles, motion, and render goals.
...
Several high-performing prompts are not just descriptions; they are layout contracts. Typical cases: museum-catalog disassembly infographics, field guides, travel/cooking cards, anatomy/science posters.
...
Data visualization mini-schema:
...
UI/UX mockup prompts succeed when they specify product context, device frame, information architecture, real copy, and data.
...
- Use original characters/products instead of existing brand or IP likenesses when creating reusable or public assets.
