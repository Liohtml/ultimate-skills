> From [openai/skills](https://github.com/openai/skills) `skills/.system/imagegen/references/sample-prompts.md` at commit `49f948f` (Apache-2.0). **Modified by ultimate-skills:** removed Codex built-in-tool / CLI-fallback mode wording. Licensed under the Apache License 2.0 (see `../LICENSE.txt`).
...
The labeled lines are prompt scaffolding, not a closed schema. `Asset type` and `Input images` are prompt-only scaffolding; the bundled scripts do not expose them as dedicated flags.

Execution details such as `quality`, `input_fidelity`, masks, output formats, aspect ratios and output paths depend on the provider and script; see `openai-cli.md` and `providers.md`.

For prompting principles (structure, specificity, invariants, iteration), see `prompting.md`.
