> From [openai/skills](https://github.com/openai/skills) `skills/.system/imagegen/references/prompting.md` at commit `49f948f` (Apache-2.0). **Modified by ultimate-skills:** made provider-neutral, removed Codex built-in-tool / CLI-fallback mode wording, added cross-references. Licensed under the Apache License 2.0 (see `../LICENSE.txt`).

# Prompting best practices

These principles apply to every provider (GPT Image, Gemini / Nano Banana, FLUX, Imagen, Ideogram, Seedream, ...) and to any image tool or MCP server in the session.

This file is about prompt structure, specificity, and iteration. Execution controls such as `quality`, `input_fidelity`, masks, aspect ratio, output format, and output paths live in `openai-cli.md` and `providers.md`. For the open-model world (negative prompts, CFG, LoRAs, ControlNet) see `model-agnostic-prompting.md`; for dense text, infographics, UI mockups and research figures see `prompt-craft.md`.
...
## Execution controls
- `quality`, `input_fidelity`, explicit masks, aspect ratio / size, output format, and output paths are request parameters, not prompt text. Saying "4K" or "16:9" inside a prompt does not set the output dimensions - set them with the script flag.
- See `openai-cli.md` (OpenAI Images API) and `providers.md` (model routing, Gemini) for those controls.
...
For copy/paste prompt specs (examples only), see `sample-prompts.md`. This file focuses on principles, specificity, and iteration patterns.
