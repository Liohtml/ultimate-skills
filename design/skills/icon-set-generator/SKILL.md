...
description: Generate a cohesive, project-specific SVG icon set for a website or app - brief, grouped icon proposal (navigation, communication, trust, actions, industry-specific), a frozen style spec (grid, stroke width, caps/joins, corner radius, padding) from five presets, hand-authored SVGs in batches with consistency checks, optical corrections, style-spec.json and a self-contained preview.html. Output uses currentColor and one shared viewBox, no API key needed. Use when the user needs custom icons, an icon set, SVG icons that look consistent together, or a project where service/feature icons are implied. For raster images, illustrations, logos-as-artwork or transparent bitmap icons use image-generation; to reuse an existing icon library in UI code follow the project's design system (see interface-design).
---

# Icon Set Generator

> Adapted from [jezweb/claude-skills](https://github.com/jezweb/claude-skills) `plugins/design-assets/skills/icon-set-generator` at commit `e875a6b` (MIT, (c) 2025 Jeremy Dawes). Changes: description rewritten with cross-pointers, `compatibility` frontmatter dropped, example brief made location-neutral, reference paths made relative links.
...
A brief like "local plumber, modern feel" is enough to proceed. Don't over-interview.

### Step 2: Suggest Icons

Read [references/industry-icons.md](references/industry-icons.md) for industry-specific suggestions. Organise into groups:
...
Read [references/style-presets.md](references/style-presets.md) for full preset definitions. Pick one as starting point:
...
Read [references/svg-examples.md](references/svg-examples.md) before generating — it contains reference implementations showing the right level of complexity and how to handle common icon shapes.
...
Generate a self-contained HTML file displaying all icons for visual review. Read [references/preview-template.md](references/preview-template.md) for the template. Requirements:
...
- [references/style-presets.md](references/style-presets.md) — Detailed preset definitions and selection guidance
- [references/industry-icons.md](references/industry-icons.md) — Industry-specific icon suggestions
- [references/preview-template.md](references/preview-template.md) — HTML template for the preview page
- [references/svg-examples.md](references/svg-examples.md) — Example SVGs showing proper construction at various complexity levels
