---
name: image-to-code
description: Elite image-first website workflow that ends in implemented frontend code. For visually important web tasks, it must first generate the design image(s) itself, deeply analyze them, then implement the website to match them as closely as possible. Use when the user wants a built website/section, not just mockups (for reference images only, use imagegen-frontend-web). Requires an image-generation tool or MCP server; without one, fall back to design-taste-frontend. It must prefer large, readable, section-specific images instead of tiny compressed boards, generate fresh standalone images for sections or detail views instead of cropping old ones, avoid lazy under-generation, avoid cards-inside-cards-inside-cards UI, and keep the hero clean, spacious, readable, and visible on a small laptop.
---
...
This skill requires an image-generation tool or MCP server (Claude Code has no native image generation). If no image-generation tool is available, say so plainly, do not pretend to generate or analyze images, and fall back to the `design-taste-frontend` skill (text-based design direction) before implementing the frontend.
...
More compact multi-section composition is acceptable only when every section stays large and readable.
