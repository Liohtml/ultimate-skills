...
description: Build distinctive, animation-rich HTML slide decks as a single self-contained file on a fixed 1920x1080 stage that scales to any screen - from scratch, from notes, or by converting a .pptx - with visual style discovery (three live title-slide previews instead of abstract questions), 12 style presets, reading-vs-speaking density modes, keyboard/touch navigation, inline text editing, anti-overflow rules and optional PDF export. Use when the user wants a presentation, pitch deck, talk slides or a web version of a PowerPoint as HTML. For a native .pptx file use a PowerPoint/pptx skill; for websites and landing pages use design-taste-frontend; for motion rules inside slides see motion-craft.
---

# Frontend Slides

> Adapted from [zarazhangrui/frontend-slides](https://github.com/zarazhangrui/frontend-slides) (MIT, (c) 2025 Zara Zhang) at commit `9906a34`. Changes: the bold-template pack and the Vercel hosting script/phase are not included (previews now mix presets with a custom design); supporting files moved into `references/`, `assets/` and `scripts/`; description rewritten.
...
4. **Progressive Disclosure** — Read the lightweight style presets first; load the full HTML template and animation references only when generating.
...
**When generating, read [assets/viewport-base.css](assets/viewport-base.css) and include its full contents in every presentation.**
...
If the user explicitly names a preset, honor that as one option and generate the remaining preview slots around it.

Read [references/STYLE_PRESETS.md](references/STYLE_PRESETS.md) for preset candidates.
...
- Generate 3 previews by default: 1 safe preset from `STYLE_PRESETS.md`, 1 more expressive preset that contrasts with it, and 1 custom wildcard designed for this brief.
- For conservative or high-stakes decks, make the safe preset especially restrained and make the wildcard authoritative rather than decorative.
- For expressive decks, keep the safe preset as a readable fallback and make the wildcard adventurous, context-specific, and clearly different from both other previews.
...
Open each preview for the user (`open` on macOS, `xdg-open` on Linux, `start` on Windows) when a desktop is available; otherwise give the file paths.
...
- [assets/viewport-base.css](assets/viewport-base.css) — Mandatory CSS (include in full)
- [references/animation-patterns.md](references/animation-patterns.md) — Animation reference for the chosen feeling (the exact curve/duration rules in `motion-craft` apply)
...
1. **Extract content** — Run `python3 scripts/extract-pptx.py <input.pptx> <output_dir>` from this skill's folder (install python-pptx if needed: `pip install python-pptx`)
...
2. **Open** — Launch the file in a browser (`open` / `xdg-open` / `start`) when a desktop is available; otherwise give the path
...
After delivery, **ask the user** whether they want a PDF export. The deck is a single static HTML file: to share it as a link, it can be put on any static host the user already uses (their own site, GitHub Pages, an internal file server) - do not sign the user up for a hosting service on their behalf.
...
| --- | --- | --- |
| [references/STYLE_PRESETS.md](references/STYLE_PRESETS.md) | 12 curated visual presets with colors, fonts, and signature elements | Phase 2 (style selection) |
| [assets/viewport-base.css](assets/viewport-base.css) | Mandatory fixed-stage CSS — copy into every presentation | Phase 3 (generation) |
| [references/html-template.md](references/html-template.md) | HTML structure, JS features, code quality standards | Phase 3 (generation) |
| [references/animation-patterns.md](references/animation-patterns.md) | CSS/JS animation snippets and effect-to-feeling guide | Phase 3 (generation) |
| `scripts/extract-pptx.py` | PPTX content extraction (needs `python-pptx`) | Phase 4 (conversion) |
| `scripts/export-pdf.sh` | Export slides to PDF (needs Node.js; installs Playwright + Chromium into a temp dir) | Phase 6 (export) |
