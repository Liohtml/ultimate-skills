# Heuristic critique (score before you redesign)

> Adapted from [pbakaus/impeccable](https://github.com/pbakaus/impeccable) `skill/reference/critique.md` at commit `e0881d2` (Apache-2.0, Copyright 2025 Paul Bakaus; see [impeccable.LICENSE.txt](impeccable.LICENSE.txt)). **Modified by ultimate-skills:** kept the Nielsen scoring guide, cognitive-load assessment, P0–P3 severity scale and persona tests; removed the dual-sub-agent orchestration, the `impeccable detect` CLI, browser-overlay injection, snapshot persistence and command routing, and wrote the short procedure below. Licensed under the Apache License 2.0; see `LICENSES/` at the repository root.

Use this at the **Diagnose** step of `redesign-existing-projects` when the user wants a critique, a before/after score, or a prioritized plan instead of an immediate rewrite.

## Procedure

1. **Look first.** Inspect the running page (browser/screenshots at desktop and mobile widths) when possible; otherwise read the templates and styles holistically before line-level issues.
2. **Design specificity verdict.** Does the result feel authored for *this* product, or would an unrelated product use it unchanged? Name the category-interchangeable choices.
3. **Score the 10 heuristics** 0–4 with the guide below and fill the Design Health Score table. Mark heuristics that genuinely cannot apply as `n/a` and renormalize.
4. **Cognitive load.** Run the checklist below; report decision points with more than 4 visible options.
5. **Persona walk-through.** Pick 2–3 personas from the selection table and list the concrete elements that fail each one on the primary task.
6. **Report:** overall impression (one paragraph), 2–3 things that work, 3–5 priority issues tagged P0–P3 (What / Why it matters / Fix), persona red flags, minor observations, 2–3 provocative questions. Then ask which issues to fix first — and feed the answer into the Fix Priority of the main skill.
...
**Mode applicability**: heuristics 7 (Flexibility and Efficiency) and 10 (Help and Documentation) may be scored `n/a` on Persuade and Experience surfaces (landing pages, campaigns, portfolios, bodies of work), as may any other heuristic that genuinely cannot apply to the surface under review. Write `n/a` in the Score cell with a one-line reason, and renormalize the total to the applicable maximum (e.g. **24/32** when two heuristics are n/a) so the rating band stays proportional. Record the applicable maximum and which heuristics were scored n/a.
...
If the project documents its audience (a PRODUCT.md, brand brief, research notes), derive 1–2 additional personas from that audience and brand information:
