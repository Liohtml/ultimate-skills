---
name: minto-pyramid
description: "Restructures writing into Barbara Minto's Pyramid Principle (conclusion first, then grouped reasons, then evidence, introduced with SCQA) and audits an existing draft's logic with a Share as-is / Minor edits / Restructure verdict. Gated: asks before rewriting a draft the user only shared, refuses to impose the pyramid on timelines, runbooks and tutorials, and never adds facts. Use when writing or reviewing something that asks a reader to accept a judgment or make a decision: a recommendation, proposal, exec summary, memo, review with a verdict, or status update with an ask; or when a draft buries its point or narrates the process instead of the finding. For removing AI-sounding phrasing use humanizer; for marketing and landing-page copy use copy-editing; for PRDs use create-prd; for ADRs and technical docs use documentation-and-adrs."
---

# Minto Pyramid

> Adapted from [millwright-labs/minto-pyramid-skill](https://github.com/millwright-labs/minto-pyramid-skill/tree/49d1f584f5661f4dd47c834f5a7f9f0c4c4279d0) (MIT, Copyright (c) 2026 Millwright Labs). Changes: description rewritten for this marketplace; operation 6 (audit report) added from [references/audit-report.md](references/audit-report.md), which is adapted from tyroneross/pyramid-principle (Apache-2.0); related-skill pointers added. The upstream eval rubric is kept in [evals/](evals/README.md).
...
| 6 | **Audit report** — five structural checks and a verdict, no rewrite | The user wants a review or "is this ready to send?", not a new version |
...
Run the five checks (governing thought, SCQA, vertical logic, horizontal logic, ordering) and return a fixed-format report whose verdict is **Share as-is**, **Minor edits**, or **Restructure**, with every finding quoted from the draft. The full procedure, verdict rules, and report template are in [references/audit-report.md](references/audit-report.md). Load it before auditing. An audit the user asked for is not a rewrite: gate question 2 still means you do not hand back a restructured version unless they ask.
...
- **humanizer**: once the order is right, remove AI-sounding phrasing without changing the facts. Run this skill first, humanizer last.
- **copy-editing**: marketing and conversion copy, where the goal is persuasion rather than a decision.
- **create-prd** (product plugin) and **documentation-and-adrs** (engineering plugin): templates for those document types; use this skill on their executive summary or recommendation section.
