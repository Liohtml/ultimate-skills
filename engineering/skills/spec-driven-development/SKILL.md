...
description: Creates an engineering spec before coding, through gated phases (scope check, specify, plan, tasks, implement) covering objective, tech stack, commands, project structure, testing strategy, boundaries and success criteria. Use when starting a new project, feature, or significant change and no specification exists yet, when requirements are unclear or only a vague idea, or when a single requirement spans several independently testable capabilities and needs a capability map of modules first. For a product-facing PRD (problem, segments, value proposition, release plan) use create-prd; to break an existing spec into ordered tasks use planning-and-task-breakdown; to record a single architecture decision use documentation-and-adrs.
---
...
Execute tasks one at a time following `incremental-implementation` and `test-driven-development`. Use `context-engineering` (upstream agent-skills, not included here: https://github.com/addyosmani/agent-skills) to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
