...
description: Structures git workflow practices. Use when committing, branching, writing commit messages, resolving merge conflicts, splitting a messy working tree into clean atomic commits, opening a pull request, pushing to a remote, bisecting a regression, or organizing work across multiple parallel streams or worktrees. Use when cutting a release, choosing a semantic version bump, tagging, or writing a changelog. For reviewing the code in a PR use code-review-and-quality.
---
...
Write the entry in the same change that makes the change, while the impact is fresh — not reconstructed from commit archaeology at release time. Breaking changes get a migration note and a deprecation window (follow the `deprecation-and-migration` skill — upstream agent-skills, not included here: https://github.com/addyosmani/agent-skills); shipping the actual release is the `shipping-and-launch` skill's job (upstream agent-skills, not included here: https://github.com/addyosmani/agent-skills) — this section is the versioning contract that feeds it.
