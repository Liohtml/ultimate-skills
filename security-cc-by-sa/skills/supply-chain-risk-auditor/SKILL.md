...
description: "Audits a project's dependencies for supply-chain risk: version-matched advisories for direct dependencies and the full lockfile tree (npm, PyPI, Go), abandoned or archived upstreams, npm publisher concentration, and install-time script execution. Two deterministic scripts do the measuring; unavailable data is never scored as clean. Use when asked to audit dependencies, assess supply-chain or third-party package risk, or review a dependency tree before an engagement. For application source-code vulnerabilities use static-vuln-scan; for GitHub Actions supply-chain pinning use gha-security-review (both in the MIT security plugin)."
---

# Supply Chain Risk Auditor

> Adapted from [trailofbits/skills](https://github.com/trailofbits/skills/tree/32e34f8173796e3566a51aee877dc96bc5191f64/plugins/supply-chain-risk-auditor) (CC-BY-SA-4.0). Modified by ultimate-skills; this whole plugin is licensed CC-BY-SA-4.0. See the change note at the end.
...
Audit only projects you own or are authorized to assess. The scripts read
manifest and lockfile metadata and query public registries and advisory
databases; they never install, build, or execute the project or its
dependencies, and make no requests against the project's own infrastructure.
Requires Python 3.11+ (stdlib only) and, for full GitHub coverage, an
authenticated `gh` CLI. Set no secrets in argv.
...
3. Collect, then render. Put outputs somewhere outside the audited repository unless
   ...
   ```sh
   uv run ${CLAUDE_SKILL_DIR}/scripts/collect.py <project-dir> --json <out-dir>/findings.json
   uv run ${CLAUDE_SKILL_DIR}/scripts/render.py <out-dir>/findings.json --out <out-dir>/report.md
   ```
...
Modified from upstream commit `32e34f8`: `{baseDir}` rewritten to `${CLAUDE_SKILL_DIR}`; the `report-writing:writing-style` cross-reference removed; an explicit authorization / read-only / no-execution note and the Python and `gh` prerequisites added; the bundled `test_*.py`, `uv.lock`, `agents/openai.yaml` and Trail of Bits brand asset removed from the skill directory; `allowed-tools` frontmatter dropped. The four collector/renderer scripts (`collect.py`, `sources.py`, `model.py`, `render.py`) and their measurement discipline are unchanged.
