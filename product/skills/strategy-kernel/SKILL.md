...
description: "Build or audit a strategy with Richard Rumelt's kernel (Good Strategy Bad Strategy): an honest diagnosis of the critical challenge, a guiding policy that rules options out, and coherent, resource-backed actions. Detects the four hallmarks of bad strategy (fluff, not facing the challenge, goals mistaken for strategy, dog's-dinner or blue-sky objectives) with the negation and paste-into-a-competitor's-deck tests, finds sources of power (leverage, proximate objectives, chain-link weak spots), and scores 0-10 on an 8-row diagnostic. Use when writing an annual or product strategy, turning a goal or OKR list into a strategy, asking whether a document is actually a strategy, or auditing a strategy deck for fluff. To attack the assumptions of a plan that already is a strategy use strategy-red-team; for launch risk use pre-mortem; for choosing a market category use product-positioning."
license: MIT
metadata:
  author: wondelai
  version: "1.2.0"
...
> Adapted from [wondelai/skills `good-strategy-bad-strategy`](https://github.com/wondelai/skills/tree/c172996495bed0fcd26896a9416b2093fd7073f0/good-strategy-bad-strategy) (MIT, Copyright (c) 2025 Wondel.ai sp. z o.o.). Changes: renamed to `strategy-kernel`, description rewritten, pointers to upstream sibling skills replaced with this marketplace's skills, affiliate links removed. The framework is Richard Rumelt's, summarized in the upstream authors' own words; read the book for the full treatment.
...
- **strategy-red-team**: after the kernel exists, attack its load-bearing assumptions and get the cheapest test and kill criteria for each. Kernel first (is it a strategy?), red-team second (will it survive?).
- **pre-mortem**: launch-readiness risks for a committed plan.
- **north-star-metric** and **opportunity-solution-tree**: turn the proximate objective into a measurable outcome and a discovery tree.
- **product-positioning**: when the diagnosis is that customers do not understand what the product is or what to compare it with.
- **minto-pyramid** (writing plugin): write the kernel up conclusion-first for executives.
