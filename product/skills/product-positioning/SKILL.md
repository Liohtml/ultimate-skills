...
description: "Define or repair product positioning with April Dunford's 5-step method: competitive alternatives (including doing nothing), unique attributes that pass the only-we test, value themes via the so-what chain, best-fit customers with negative criteria, and the market category choice (head-to-head, subcategory, or new category). Produces a 10-part positioning canvas and a 0-10 score with what is needed to reach 10. Use when the user mentions positioning or repositioning, which market category to be in, category creation, a positioning canvas or workshop, launching into a crowded market, or prospects not getting what the product is. For the customer-value statement use value-proposition; for researching and profiling specific competitors use competitor-analysis; for turning positioning into page copy use copy-editing (writing plugin)."
license: MIT
metadata:
  author: wondelai
  version: "1.4.1"
...
> Adapted from [wondelai/skills `obviously-awesome`](https://github.com/wondelai/skills/tree/c172996495bed0fcd26896a9416b2093fd7073f0/obviously-awesome) (MIT, Copyright (c) 2025 Wondel.ai sp. z o.o.). Changes: renamed to `product-positioning`, description rewritten, pointers to upstream sibling skills replaced with this marketplace's skills, affiliate links removed. The method is April Dunford's, summarized in the upstream authors' own words; read the book for the full treatment.
...
- **value-proposition**: once positioning is set, write the 6-part JTBD value proposition for the best-fit segment.
- **competitor-analysis**: evidence-labelled research on named competitors; feeds Step 1 (alternatives) and Step 2 (the only-we test).
- **pricing-strategy**: the category choice sets price expectations; price inside the frame you picked.
- **interview-script**: the Step 1 happy-customer interviews on what they used before.
- **copy-editing** (writing plugin): derive and edit page copy after the canvas is agreed.
