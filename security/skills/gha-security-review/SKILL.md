...
description: >-
  Exploit-focused security review of GitHub Actions workflows and composite
  actions (.github/workflows/*.yml, action.yml): pwn requests via
  pull_request_target, ${{ }} expression injection in run blocks,
  comment-triggered commands without author checks, credential escalation,
  PR-supplied config poisoning (CLAUDE.md/AGENTS.md/Makefile read by AI or
  build steps), unpinned third-party actions, over-broad permissions and
  self-hosted runner/cache risks. Every reported finding needs a traced attack
  path by an external attacker without write access; includes an explicit
  do-not-flag list. Use when asked to "review GitHub Actions", "audit
  workflows", "check CI security" or review a PR touching .github/. For
  building or fixing pipelines use ci-cd-and-automation; for dependency and
  package risk use supply-chain-risk-auditor (security-cc-by-sa plugin); for
  application source code use static-vuln-scan.
---
...
> Includes material adapted from [getsentry/skills](https://github.com/getsentry/skills/tree/c2f99a5b04b4cd992ec3022d7c2c3e23e938d241/skills/gha-security-review) (Apache-2.0). Modified by ultimate-skills: see `LICENSE.txt` and the change note at the end.
...
**This is a read-only review.** Review only repositories the user owns or is authorized to assess. PoC sketches are written descriptions for the report; never open PRs, post comments, push branches, or trigger workflows against a real repository to "prove" a finding. The payload examples in `references/` document real-world attacks so they can be recognized — they are not to be executed.
...
Modified from upstream commit `c2f99a5`: description rewritten with cross-pointers; explicit read-only/authorization paragraph added to Scope; `allowed-tools` frontmatter dropped. The reference files are unchanged.
