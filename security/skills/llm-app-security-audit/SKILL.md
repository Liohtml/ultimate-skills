...
description: >-
  Defensive code audit of an application's AI/LLM features for prompt injection
  and agent security. Maps the AI attack surface (LLM SDKs, agent frameworks,
  MCP tool registrations), then for each integration records the six questions
  (system prompt, user input, external/RAG data, actions the model can take,
  downstream use of output, permission context) and audits prompt construction,
  output handling, tool/agent boundaries, prompt leaking and AI permission
  boundaries — including the cross-privilege class where a low-privilege user
  poisons data a high-privilege AI session reads. Maps findings to the OWASP LLM
  Top 10. Use when auditing an existing AI feature, chatbot, RAG pipeline, agent
  or MCP server for injection and privilege bugs. For design-time threat
  modeling of a new AI feature use threat-model; for hardening AI code while
  building use security-and-hardening; for scanning a skill/plugin file use
  skill-scanner; for general source-code vulns use static-vuln-scan.
---

# LLM Application Security Audit (prompt injection & agent security)

> Includes material adapted from [briiirussell/cybersecurity-skills](https://github.com/briiirussell/cybersecurity-skills/tree/c9ade0349f8ecd5c9c68a82a05d2b6012c5fa1d7/skills/prompt-injection) (MIT). Modified by ultimate-skills: see `LICENSE.txt` and the change note at the end. OWASP LLM Top 10 IDs used for mapping are the OWASP GenAI Security Project's published list.

Audit applications that use AI features, LLM integrations, or AI agents for prompt injection, privilege escalation, and authorization bypass vulnerabilities. This is defensive code review: only audit code the user owns or is authorized to review, provide remediation for every finding, and do not craft payloads for use against systems the user does not control.

Cross-references: use `threat-model` for design-time AI risk modeling on a new feature (before this skill applies); `security-and-hardening` for the build-time controls behind each finding; `skill-scanner` when the artifact under review is itself an agent skill or plugin.
...
Modified from upstream commit `c9ade03`: renamed from `prompt-injection` to `llm-app-security-audit` for clarity; description rewritten with cross-pointers into this marketplace; cross-references to sibling skills not imported here (`owasp-audit`, `api-audit`, `ai-risk-management`) rewritten to point at this marketplace's `threat-model`, `security-and-hardening` and `skill-scanner`; `allowed-tools` frontmatter dropped. The audit methodology (six-question surface map, three injection classes, tool/agent and permission-boundary audits) is kept intact.
