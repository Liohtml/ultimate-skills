...
> Part of the `threat-model` skill. Adapted from [anthropics/defending-code-reference-harness](https://github.com/anthropics/defending-code-reference-harness/tree/d3bea6b5793b5f3d59a75ebe69a58efa88383145/.claude/skills/threat-model) `threat-model/bootstrap.md` (Apache-2.0, see `../LICENSE.txt`); modified by ultimate-skills (see the change note at the end of `../SKILL.md`).
...
> with `cat ${CLAUDE_SKILL_DIR}/references/bootstrap.md` via Bash.
...
`<target-dir>`) so a fresh bootstrap session can resume
...
`python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py load ./.threat-model-state`

- `status == "absent"` OR `"complete"`, OR `--fresh` in the invocation arguments →
  **fresh start.** Bash:
  `python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py reset ./.threat-model-state`,
...
1. Write tool → `./.threat-model-state/_chunk.tmp` containing the
   stage's output JSON.
2. Bash → `python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py save ./.threat-model-state <N> <name> --key stage --from ./.threat-model-state/_chunk.tmp`

**End of run.** After writing `<target-dir>/THREAT_MODEL.md`, Bash:
`python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py done ./.threat-model-state 5 --key stage`
...
parallel. Spawn the agents below **in a single batch** with the subagent (Task/Agent) tool so
...
Then Bash:
`python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py save ./.threat-model-state 1 swarm --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py save ./.threat-model-state 2 synthesize --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py save ./.threat-model-state 3 generalize --key stage --from ./.threat-model-state/_chunk.tmp`
...
Then Bash:
`python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py save ./.threat-model-state 4 gap-fill --key stage --from ./.threat-model-state/_chunk.tmp`
...
These seed a later `threat-model` interview mode with `--seed THREAT_MODEL.md`
pass.
...
2. For each remaining section (`## 2. Assets`, `## 3. Entry points`,
   ...
   - Write tool → `./.threat-model-state/_chunk.tmp` containing that ONE
     section's markdown.
   - Bash:
     `python3 ${CLAUDE_SKILL_DIR}/scripts/checkpoint.py append ./.threat-model-state/THREAT_MODEL.md --from ./.threat-model-state/_chunk.tmp`
...
4. Stage-3b sibling locations as candidate leads for the `static-vuln-scan`
   skill (pass them as `--focus` areas).
